...
void log_in() {
    ...
    for(int i =0 ; i<=trying ; i++){
        ...
        if(ry == false){
            cout << endl << "welcome in our game store !!!" << "\n" << "\n";
        }
...
void options(){
    ...
    cout<<endl<<"choose the game (enter the number of the game) :  ";
    cin>>answer;
    cout<<"\n"<<"\n";
}

void log_out(){//log out function
    char ans;
...
void attack(){
    ...
    par = rand() % 5;
    computer = arr[par];
    cout << endl << endl << "Now choose monster : (1 or 2 or 3 or 4) : ";
    cin >> choice;
    while (true) {
        if (choice == 1) {
            player = "big spider";
...
int main() {
    ...
    while (i == 0) {// make sure that the log_in function will run only one time
        log_in();
        i++;
    }
    while (n!=0) {
        if (log == 1) {//display options
            options();
            if (answer == 1) {//first game (guess game)
                guess();
            } else if (answer == 2) {// second game (test your info)
                test();
            } else if (answer == 3) {// Third game(monsters game)
                attack();
            } else if (answer == 4) {//fourth game(try to survive)
                SPS();
            } else if (answer == 5) {
                n = false;
            } else {
